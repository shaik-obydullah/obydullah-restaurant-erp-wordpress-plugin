...
class Obydullah_ERP_Employees
{
    ...
    {
        global $wpdb;
        $this->table = $wpdb->prefix . 'orerp_employees';
    ...
    public function orerp_get_employees($args = [])
    {
        ...
        $filters = $this->orerp_build_employee_filters($args['search'], $args['branch_id'], $args['active']);

        $offset = ($args['page'] - 1) * $args['per_page'];

        $total = intval($wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$this->table} e {$filters['where']}",
            $filters['args']
        )));
        ...
        $results = $wpdb->get_results($wpdb->prepare(
            ...
            LIMIT %d OFFSET %d",
            array_merge($filters['args'], [$args['per_page'], $offset])
        ));
    ...
    private function orerp_build_employee_filters($search, $branch_id, $active)
    {
        global $wpdb;

        $has_search = !empty($search);
        $has_branch = intval($branch_id) > 0;
        $has_active = $active !== '';

        $like = '%' . $wpdb->esc_like($search) . '%';

        if ($has_search && $has_branch && $has_active) {
            return [
                'where' => 'WHERE (e.employee_code LIKE %s OR e.position LIKE %s) AND e.branch_id = %d AND e.is_active = %d',
                'args'  => [$like, $like, intval($branch_id), intval($active)],
            ];
        }

        if ($has_search && $has_branch) {
            return [
                'where' => 'WHERE (e.employee_code LIKE %s OR e.position LIKE %s) AND e.branch_id = %d',
                'args'  => [$like, $like, intval($branch_id)],
            ];
        }

        if ($has_search && $has_active) {
            return [
                'where' => 'WHERE (e.employee_code LIKE %s OR e.position LIKE %s) AND e.is_active = %d',
                'args'  => [$like, $like, intval($active)],
            ];
        }

        if ($has_branch && $has_active) {
            return [
                'where' => 'WHERE e.branch_id = %d AND e.is_active = %d',
                'args'  => [intval($branch_id), intval($active)],
            ];
        }

        if ($has_search) {
            return [
                'where' => 'WHERE (e.employee_code LIKE %s OR e.position LIKE %s)',
                'args'  => [$like, $like],
            ];
        }
        ...
        if ($has_branch) {
            return [
                'where' => 'WHERE e.branch_id = %d',
                'args'  => [intval($branch_id)],
            ];
        }

        if ($has_active) {
            return [
                'where' => 'WHERE e.is_active = %d',
                'args'  => [intval($active)],
            ];
        }
